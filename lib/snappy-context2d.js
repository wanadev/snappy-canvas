...
class SnappyContext2D {
    ...
    render() {
        ...
        var _posx = (x, cs, scaleX = cs.globalScale) => ((x+cs.tx)*scaleX|0)+((cs.lw*isStroke)%2/2);
        var _posy = (y, cs, scaleY = cs.globalScale) => ((y+cs.ty)*scaleY|0)+((cs.lw*isStroke)%2/2);
        var _size = (s, cs) => s*cs.globalScale|0;
        var _sizex = (s, cs) => s*cs.scale.x|0;
        var _sizey = (s, cs) => s*cs.scale.y|0;
        var _nop = (v, cs) => v;
        ...
        var operations = {
            ...
            fillRect : {fn: function(operation, operationName, ...values) {
                ...
                ctx.save();
                ctx.fillRect(
                    _posx(values[0],contextStatus),
                    _posy(values[1],contextStatus), 
                    _sizex(values[2],contextStatus),
                    _sizey(values[3],contextStatus)
                    );
                ctx.restore();
            }},
            ...
            fillText: {fn: function(operation, operationName, ...values) {
                ...
                ctx.translate(contextStatus.tx / contextStatus.scale.defaultX, contextStatus.ty / contextStatus.scale.defaultY);
            ...
            strokeText: {fn: function(operation, operationName, ...values) {
                ...
                if (!options.scaleLineWidth) {
                    ctx.lineWidth = contextStatus.lw / contextStatus.scale.x;
                }
                ctx[operationName](...values);
                ctx.restore();
            }},
            ...
            lineWidth: {fn: function(operation, operationName, ...values) {
                var lineWidth;
                if (options.scaleLineWidth) {
                    lineWidth = (values[0] * contextStatus.globalScale) | 0;
                } else {
            ...
            lineDashOffset: {fn: function(operation, operationName, ...values) {
                ctx.lineDashOffset = values[0] * (contextStatus.scaleDashesWidth ? contextStatus.globalScale : 1) | 0;
            }},
            ...
            scale: {fn : function(operation, operationName, ...values) {
                var scaleX = values.length > 0 ? values[0] : 1;
                var scaleY = values.length > 1 ? values[1] : 1;
                contextStatus.scale = {
                    defaultX : scaleX,
                    defaultY : scaleY,
                ...
                switch (values.length) {
                    case 3:
                        _contextOperationCall(ctx, operationName,
                            values[0],                              // Image
                            _posx(values[1], contextStatus, contextStatus.scale.x),        // dx
                            _posy(values[2], contextStatus, contextStatus.scale.y),        // dy
                    ...
                    case 9:
                        _contextOperationCall(ctx, operationName,
                            ...
                            values[3],                              // sWidth
                            values[4],                              // sHeight
                            _posx(values[5], contextStatus, contextStatus.scale.x),        // dx
                            _posy(values[6], contextStatus, contextStatus.scale.y),        // dy
