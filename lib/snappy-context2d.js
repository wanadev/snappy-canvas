...
class SnappyContext2D {
    ...
    render() {
        ...
        var _posx = (x,cs) => x;
        var _posy = (y,cs) => y;
        var _adjust = (x, y, contextStatus) => {
            const worldMatrix = contextStatus.currentMatrix.clone();
            const oppositeWorldMatrix = Matrix3D.opposite(worldMatrix.clone());
            let vector2 = {
            ...
            if (!operation.isPath) {
                ...
                for (let i = 0 ; i < values.length ; i++) {
                    if ( operation.args[i].name == "_posx" && operation.args[i + 1 ].name == "_posy") {
                        const vector2 = _adjust(values[i], values[i + 1], contextStatus);
                        args[i] = vector2.x;
                        args[i + 1] = vector2.y;
                        i += 1;
                    }
